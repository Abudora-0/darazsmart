...
export default function CartPage() {
  ...
  return (
    <div className="px-4 py-6 sm:px-6">
      <h1 className="mb-5 text-xl font-bold text-[#1a1730]">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-center gap-4 rounded-3xl bg-white p-4 ring-1 ring-black/5"
            >
              <Link
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1">
                <Link
                  href={`/product/${item.id}`}
                  className="line-clamp-2 text-sm font-semibold text-[#1a1730] hover:text-brand-600"
                >
                  {item.title}
                </Link>
                {item.seller && (
        ...
        <div className="h-fit rounded-3xl bg-white p-6 ring-1 ring-black/5 lg:sticky lg:top-6">
          <h2 className="mb-4 font-bold text-[#1a1730]">Summary</h2>
